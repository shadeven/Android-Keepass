...
public class ImporterV4 extends Importer {
	...
	protected KDBV4 createDB() {
		return new KDBV4();

	}
	
	@Override
	public KDBV4 openDatabase(InputStream inStream, String password, String keyfile) throws IOException,
			InvalidDBException {
	...
	private Stack<GroupV4> ctxGroups = new Stack<GroupV4>();
	private GroupV4 ctxGroup = null;
	private EntryV4 ctxEntry = null;
	...
	private EntryV4 ctxHistoryBase = null;
	private PwDeletedObject ctxDeletedObject = null;
	private UUID customIconID = KDBV4.UUID_ZERO;
	...
	private KdbContext ReadXmlElement(KdbContext ctx, XmlPullParser xpp) throws XmlPullParserException, IOException, InvalidDBException {
		...
		case Root:
			if ( name.equalsIgnoreCase(ElemGroup) ) {
				...
				db.rootGroup = new GroupV4();
				ctxGroups.push((GroupV4)db.rootGroup);
	...
	private KdbContext EndXmlElement(KdbContext ctx, XmlPullParser xpp) throws XmlPullParserException {
		...
		if ( ctx == KdbContext.KeePassFile && name.equalsIgnoreCase(ElemDocNode) ) {
			...
			if ( ctxGroup.uuid == null || ctxGroup.uuid.equals(KDBV4.UUID_ZERO) ) {
			...
			if ( ctxEntry.uuid == null || ctxEntry.uuid.equals(KDBV4.UUID_ZERO) ) {
