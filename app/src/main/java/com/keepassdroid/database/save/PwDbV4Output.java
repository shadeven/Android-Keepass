...
import com.keepassdroid.utils.Xml;
...
import com.keepassdroid.database.KDBV4.MemoryProtectionConfig;
...
import com.keepassdroid.database.model.EntryV4;
import com.keepassdroid.database.model.EntryV4.AutoType;
...
public class PwDbV4Output extends PwDbOutput {
	...
	protected PwDbV4Output(KDBV4 pm, OutputStream os) {
		super(os);
	...
	private class GroupWriter extends GroupHandler<Group> {
		private Stack<GroupV4> groupStack;
		
		public GroupWriter(Stack<GroupV4> gs) {
			groupStack = gs;
		}

		@Override
		public boolean operate(Group g) {
			GroupV4 group = (GroupV4) g;
			assert(group != null);
	...
	private class EntryWriter extends EntryHandler<Entry> {

		@Override
		public boolean operate(Entry e) {
			EntryV4 entry = (EntryV4) e;
			assert(entry != null);
	...
	private void outputDatabase(OutputStream os) throws IllegalArgumentException, IllegalStateException, IOException {
		binPool = new BinaryPool((GroupV4)mPM.rootGroup);
		...
		GroupV4 root = (GroupV4) mPM.rootGroup;
		xml.startTag(null, ElemRoot);
		startGroup(root);
		Stack<GroupV4> groupStack = new Stack<GroupV4>();
	...
	private void startGroup(GroupV4 group) throws IllegalArgumentException, IllegalStateException, IOException {
	...
	private void writeEntry(EntryV4 entry, boolean isHistory) throws IllegalArgumentException, IllegalStateException, IOException {
		assert(entry != null);
	...
	private void writeList(String name, Map<String, String> customData) throws IllegalArgumentException, IllegalStateException, IOException {
		...
		for (Map.Entry<String, String> pair : customData.entrySet()) {
	...
	private void writeList(String name, List<EntryV4> value, boolean isHistory) throws IllegalArgumentException, IllegalStateException, IOException {
		assert(name != null && value != null);
		
		xml.startTag(null, name);
		
		for (EntryV4 entry : value) {
	...
	private void writeBinPool() throws IllegalArgumentException, IllegalStateException, IOException {
		...
		for (Map.Entry<String, ProtectedBinary> pair : binPool.entrySet()) {
			xml.startTag(null, ElemBinary);
