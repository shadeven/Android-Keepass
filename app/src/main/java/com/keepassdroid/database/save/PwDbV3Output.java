...
public class PwDbV3Output extends PwDbOutput {
	...
	public PwDbV3Output(KDBV3 pm, OutputStream os) {
		super(os);
	...
	public void outputPlanGroupAndEntries(OutputStream os) throws PwDbOutputException  {
		...
		// Groups
		List<Group> groups = mPM.getGroups();
		for ( int i = 0; i < groups.size(); i++ ) {
			GroupV3 pg = (GroupV3) groups.get(i);
		...
		for (int i = 0; i < mPM.entries.size(); i++ ) {
			EntryV3 pe = (EntryV3) mPM.entries.get(i);
	...
	private void sortGroupsForOutput() {
		...
		List<Group> roots = mPM.getGrpRoots();
		for ( int i = 0; i < roots.size(); i++ ) {
			sortGroup((GroupV3) roots.get(i), groupList);
		}
		
		mPM.setGroups(groupList);
	}
	
	private void sortGroup(GroupV3 group, List<Group> groupList) {
		// Add current group
		groupList.add(group);
		
		// Recurse over children
		for ( int i = 0; i < group.childGroups.size(); i++ ) {
			sortGroup((GroupV3) group.childGroups.get(i), groupList);
		}
	}
