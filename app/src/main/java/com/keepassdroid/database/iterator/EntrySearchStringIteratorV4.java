...
public class EntrySearchStringIteratorV4 extends EntrySearchStringIterator {
	...
	public EntrySearchStringIteratorV4(EntryV4 entry) {
	...
	public EntrySearchStringIteratorV4(EntryV4 entry, SearchParametersV4 sp) {
		this.sp = sp;
		setIterator = entry.strings.entrySet().iterator();
		advance();
	}
	...
	private boolean searchInField(String key) {
		if (key.equals(EntryV4.STR_TITLE)) {
			return sp.searchInTitles;
		} else if (key.equals(EntryV4.STR_USERNAME)) {
			return sp.searchInUserNames;
		} else if (key.equals(EntryV4.STR_PASSWORD)) {
			return sp.searchInPasswords;
		} else if (key.equals(EntryV4.STR_URL)) {
			return sp.searchInUrls;
		} else if (key.equals(EntryV4.STR_NOTES)) {
