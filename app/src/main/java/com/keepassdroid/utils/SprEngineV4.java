...
public class SprEngineV4 extends SprEngine {
	...
	public class TargetResult {
		public EntryV4 entry;
		public char wanted;
		
		public TargetResult(EntryV4 entry, char wanted) {
			this.entry = entry;
			this.wanted = wanted;
		}
	}
	...
	public String compile(String text, Entry entry, KDB database) {
		SprContextV4 ctx = new SprContextV4((KDBV4)database, (EntryV4)entry);
	...
	public TargetResult findRefTarget(String fullRef, SprContextV4 ctx) {
		...
		if (list.size() > 0) { 
			return new TargetResult((EntryV4)list.get(0), wanted);
        }
	...
	private String fillRefsUsingCache(String text, SprContextV4 ctx) {
		for (Map.Entry<String, String> entry : ctx.refsCache.entrySet()) {
